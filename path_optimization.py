...
class PathOptimisation:

    
    def __init__(self,max_gap=5):
        self.max_gap = max_gap
        if self.max_gap<1:
            print("ERROR : MAXIMUM GAP SHOULD BE POSITIVE AND NOT NULL ")
    
    def build_graph(self,my_list):
        '''
    ...
    def dag_shortest_path(self,my_graph):
        '''
        ...
        for i in range(num_vertice):
            myvertice.append([i,99999,99999])
        myvertice[0][1]=0
    ...
    def main_function(self,my_array):
        """
        ...
        for i in range(len(my_array)):  # loop on the entire array
            ...
            if my_array[i]>-1:          # if the relevance of an offer is positive
                ...
                if abs(local2-local1)>self.max_gap:  # test if the distance to the previous positive offer higher than 5
                    ...
                    my_graph = self.build_graph(my_problem)  # build graph associated to the problem
                    my_solution = self.dag_shortest_path(my_graph)  # find shortest path
